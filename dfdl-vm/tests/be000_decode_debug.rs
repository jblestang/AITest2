...
fn be000_y_count_from_tdml_bytes() {
    let path = "/workspace/third_party/daffodil/daffodil-test/src/test/resources/org/apache/daffodil/section14/unordered_sequences/BE.tdml";
    ...
    fn y_len(v: &DfdlValue) -> usize {
        let DfdlValue::Sequence(outer) = v else { return 0 };
        let inner = outer.fields.get("seq").or_else(|| outer.fields.values().next());
        let Some(DfdlValue::Sequence(seq)) = inner else { return 0 };
