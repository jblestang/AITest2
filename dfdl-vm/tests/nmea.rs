...
fn nmea_spec(xsd: &str) -> DfdlSpec {
    let resolver = SchemaResolver::new()
        .with_bundled("nmea_types.xsd", include_str!("fixtures/nmea_types.xsd"));
...
fn nmea_gpgga_typed_decode() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gpgga.xsd"));
    let decoded = spec.decode(GPGGA_SAMPLE).expect("decode");
    ...
    assert_eq!(
        body.field("altitudeUnits"),
        Some(&DfdlValue::String("M".into()))
    );
    assert_eq!(
        body.field("dgpsAge"),
...
fn nmea_gprmc_typed_decode_and_round_trip() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gprmc.xsd"));
    ...
    assert_eq!(
        body.field("magVarHemisphere"),
        Some(&DfdlValue::String("W".into()))
    );
    let encoded = spec.encode(&decoded).expect("encode");
...
fn nmea_aivdm_typed_decode_and_round_trip() {
    let spec = nmea_spec(include_str!("fixtures/nmea_aivdm.xsd"));
...
fn nmea_gll_typed_decode_and_round_trip() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gll.xsd"));
...
#[test]
fn nmea_typed_rejects_invalid_latitude_hemisphere() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gpgga.xsd"));
    let bad = b"$GPGGA,123519,4807.038,X,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*00\r\n";
    assert!(spec.decode(bad).is_err());
}

#[test]
fn nmea_typed_rejects_invalid_longitude_format() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gll.xsd"));
    let bad = b"$GPGLL,4807.038,N,1131.000,E,123519,A,A*00\r\n";
    assert!(spec.decode(bad).is_err());
}

#[test]
fn nmea_typed_rejects_invalid_status() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gprmc.xsd"));
    let bad = b"$GPRMC,123519,X,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W*00\r\n";
    assert!(spec.decode(bad).is_err());
}

#[test]
fn nmea_typed_rejects_invalid_date_length() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gprmc.xsd"));
    let bad = b"$GPRMC,123519,A,4807.038,N,01131.000,E,022.4,084.4,23039,003.1,W*00\r\n";
    assert!(spec.decode(bad).is_err());
}

#[test]
fn nmea_typed_rejects_invalid_checksum() {
    let spec = nmea_spec(include_str!("fixtures/nmea_gpgga.xsd"));
    let bad = b"$GPGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*G\r\n";
    assert!(spec.decode(bad).is_err());
}

#[test]
fn nmea_typed_rejects_invalid_ais_channel() {
    let spec = nmea_spec(include_str!("fixtures/nmea_aivdm.xsd"));
    let bad = b"!AIVDM,1,1,,C,15M67FC000G?l`nQ@`WplQ@T400,0*00\r\n";
    assert!(spec.decode(bad).is_err());
}
