...
#[cfg(test)]
use decimal::parse_mantissa_decimal;
pub(crate) use decimal::{
    decimal_is_zero, decimal_to_string, parse_decimal_value, parse_sci_or_decimal, Decimal,
    SciParts,
};
pub(crate) use rounding::{
    apply_rounding_increment_to_decimal, round_to_max_fraction_digits, trim_insignificant_fraction,
};
            ...
            '0' | '#' => {
                ...
                if saw_decimal {
                    ...
                    while frac_idx < frac_digits.len() && emitted < max {
                        ...
                        if min == 0 && remaining.iter().all(|&d| d == 0) && emitted >= min {
                            break;
                        }
...
mod tests {
    ...
    fn pattern_double_zero_suffix() {
        ...
        let out = format_standard_text_number("1235.5", "##00", &fmt, rounding).unwrap();
        assert_eq!(out, "1236");
    }
