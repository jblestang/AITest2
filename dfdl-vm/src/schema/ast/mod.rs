...
pub struct DfdlProps {
    ...
    /// Set when format context defaults have been applied for this type's schema document.
    pub format_context_finalized: bool,
    ...
    /// Raw expression string for occursCount (e.g. `{ if ($ex:capture) then 1 else 0 }`).
    ...
    pub fn builtin_for_simple_base(&self, base: &SimpleBase) -> Option<BuiltinType> {
        match base {
            ...
            SimpleBase::Restriction { base, .. } => match base {
                ...
                RestrictionBase::Named(name) => self.types.get(name).and_then(|def| {
                    if let TypeDef::Simple { base: inner, .. } = def {
                        self.builtin_for_simple_base(inner)
                    } else {
                        None
    ...
    pub fn effective_simple_type_props(&self, name: &TypeName) -> Option<DfdlProps> {
        ...
        let mut out = match base {
            SimpleBase::Restriction {
                ...
                ..
            } => self.effective_simple_type_props(parent).unwrap_or_default(),
            SimpleBase::Restriction {
